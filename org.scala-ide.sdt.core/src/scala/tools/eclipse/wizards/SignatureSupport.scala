...
object SignatureSupport extends JavaTokenParsers {
  ...
  def typeSignature: Parser[String] = (primitive
    | "T" ~> ident <~ ";" ^^ { i => i }
  ...
  def resolvedClassTypeSignature: Parser[String] = ( //("L" ~> ident ~ optionalTypeArguments ~ qualifiedName ~ optionalTypeArguments <~ ";")  ^^ { case a~b~c~d => a + b.toString + c + d}
  ("L" ~> qualifiedName ~ optionalTypeArguments <~ ";") ^^ { case a ~ b => a + b }
    | (optionalTypeParameters <~ "L" ~> qualifiedName <~ ";") ^^ { case l => l.mkString(", ") })

  def unresolvedClassTypeSignature: Parser[String] = ( //("Q" ~> ident ~ optionalTypeArguments ~ qualifiedName ~ optionalTypeArguments <~ ";") ^^ { case a~b~c~d => a + b.toString + c + d}
  ...
  def typeArgument: Parser[String] = (typeSignature
    | "*" ^^ { i => "_" }
